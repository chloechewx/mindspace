...
export const JournalPage: React.FC = () => {
  ...
  const handleGenerateInsights = async (entryId: string, isRegeneration: boolean = false) => {
    ...
    // 🔍 DEBUG: Log what we're passing to the AI service
    console.log('=== JOURNAL PAGE DEBUG ===');
    console.log('Entry ID:', entryId);
    console.log('Entry Object:', entry);
    console.log('Passing to AI:');
    console.log('  - mood:', entry.mood);
    console.log('  - gratitude:', entry.gratitude);
    console.log('  - goals:', entry.goals);
    console.log('  - content:', entry.content);
    console.log('  - isRegeneration:', isRegeneration);
    console.log('=== END JOURNAL PAGE DEBUG ===');

    setGeneratingInsights(entryId);
    try {
      // Generate insights using Gemini API with regeneration flag
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      ...
      {/* Calendar View - keeping existing code */}
      {activeView === 'calendar' && (
        <div className="space-y-6">
          {!isAuthenticated ? (
            <div className="bg-white rounded-3xl shadow-xl p-12 text-center">
      ...
      {/* Analysis View - keeping existing code */}
      {activeView === 'analysis' && (
        <div className="space-y-6">
          {!isAuthenticated ? (
            <div className="bg-white rounded-3xl shadow-xl p-12 text-center">
